...
class OrderTerminatedWebhook
{
    ...
    public function handle(OrderTerminated $event): void
    {
        ...
        $response = Http::post(settings('event_webhook_url'), [
            // Message
            'content' => 'Order terminated',

            // Embeds Array
            'embeds' => [
                [
                    // Embed Title
                    'title' => 'The following order has been terminated',
     ...
     */
    public function failed(OrderTerminated $event, Throwable $exception): void
    {
        ErrorLog('OrderTerminatedWebhook', $exception->getMessage());
    }
}
