...
class GatewaysController extends Controller
{
    ...
    public function gateways(Request $request)
    {
        ...
        try {
            // add relations
            $this->applyRelations($gateways);

            // apply filters
            $this->applyFilters($gateways);

            // apply sorting
            $this->applySorts($gateways);

            // apply date filters
            $this->applyDateFilters($gateways);
        ...
        return response()->json(array_merge(
            ['status' => true],
            $gateways->paginate($request->get('paginate', 15))->toArray()
        ), 200);
    }
