...
use App\Models\EmailHistory as Email;
use Carbon\Carbon;
...
class EmailsController extends Controller
{
    ...
    public function emails(Request $request)
    {
        ...
        try {
            // add relations
            $this->applyRelations($emails);

            // apply filters
            $this->applyFilters($emails);

            // apply sorting
            $this->applySorts($emails);

            // apply date filters
            $this->applyDateFilters($emails);
        ...
        return response()->json(array_merge(
            ['status' => true],
            $emails->paginate($request->get('paginate', 15))->toArray()
        ), 200);
    }
