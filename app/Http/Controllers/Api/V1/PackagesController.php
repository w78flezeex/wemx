...
class PackagesController extends Controller
{
    ...
    public function packages(Request $request)
    {
        ...
        try {
            // add relations
            $this->applyRelations($packages);

            // apply filters
            $this->applyFilters($packages);

            // apply sorting
            $this->applySorts($packages);

            // apply date filters
            $this->applyDateFilters($packages);
        ...
        return response()->json(array_merge(
            ['status' => true],
            $packages->paginate($request->get('paginate', 15))->toArray()
        ), 200);
    }
