...
@section('title', 'Mods | ' . $order->name)

@if(settings('encrypted::pterodactyl::api_admin_key', false))
    @section('content')
        <div class="flex flex-wrap gap-4 mb-6 justify-center">
            <a href="{{ route('pterodactyl.mods.modrinth', ['order' => $order->id]) }}"
                        ...
                        <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-2 py-1">
                                <div class="flex justify-end items-center space-x-2">
                                    ...
                                    <form
                                        action="{{ route('pterodactyl.mods.toggle', ['order' => $order->id, 'server' => $server['identifier']]) }}"
                                        method="POST">
                                        @csrf
                                    ...
                                    {{-- Delete Plugin --}}
                                    <form
                                        action="{{ route('pterodactyl.mods.delete', ['order' => $order->id, 'server' => $server['identifier']]) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="name" value="{{ $plugin['name'] }}">
