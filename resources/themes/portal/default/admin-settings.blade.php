<form action="{{ route('admin.settings.store') }}" method="POST">
    ...
    <div class="row">
        ...
        <div class="form-group col-6">
            ...
            <input type="text" name="portal::default::header_image"
                value="@settings('portal::default::header_image', 'https://www.freepnglogos.com/uploads/minecraft-png/download-minecraft-characters-png-png-image-pngimg-29.png')"
                class="form-control">
        </div>

        <div class="form-group col-6">
            <label for="category">{!! __('client.default_category') !!}</label>
            <select class="form-control select2 select2-hidden-accessible" name="portal::default_category" tabindex="-1"
                aria-hidden="true">
                @foreach (Categories::get() as $category)
                    <option value="{{ $category->link }}" @if (settings('portal::default_category') == $category->link) selected @endif>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
